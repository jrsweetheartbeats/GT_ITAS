...
from ..models import Attachment, Project, ReviewFinding, ReviewFindingHistory, ReviewRun, ReviewStep, Workpaper, WorkpaperVersion
from .privacy import project_redaction_terms, redact_payload, redact_text
from .timeliness import days_after
...
def add_finding(
    ...
    finding = ReviewFinding(
        ...
        workpaper_version_id=version.id if version else None,
        rule_code=rule_code,
        severity=severity,
        target=target,
        issue=issue,
    ...
    db.add(
        ReviewFindingHistory(
            ...
            comment="自动复核发现",
            change_summary=(
                f"规则 {rule_code} 创建问题并自动分派给底稿编制人"
                if assignee_user_id
                else f"规则 {rule_code} 创建问题，待复核人分派"
            ),
        )
    )
...
def run_internal_review(db: Session, run: ReviewRun) -> int:
    ...
    workpaper_stmt = select(Workpaper).where(Workpaper.project_id == project.id)
    attachment_stmt = select(Attachment).where(Attachment.project_id == project.id)
    if run.workpaper_id is not None:
        workpaper_stmt = workpaper_stmt.where(Workpaper.id == run.workpaper_id)
        attachment_stmt = attachment_stmt.where(Attachment.workpaper_id == run.workpaper_id)
    workpapers = db.execute(workpaper_stmt).scalars().all()
    attachments = db.execute(attachment_stmt).scalars().all()
    count = 0

    if run.workpaper_id is None and not project.audit_year:
    ...
    for att in attachments:
        ...
        if att.file_path and not Path(att.file_path).expanduser().exists():
            ...
            count += 1
        if att.file_path and project.audit_scope_start and project.audit_scope_end:
            path = Path(att.file_path).expanduser()
            if path.exists() and path.is_file():
                stat = path.stat()
                # macOS/Linux ctime is metadata-change time, not a reliable business
                # creation date. It remains an audit clue and must be corroborated.
                timestamps = {
                    "文件创建/元数据变更时间": datetime.fromtimestamp(stat.st_ctime).date(),
                    "文件修改时间": datetime.fromtimestamp(stat.st_mtime).date(),
                }
                outside = {label: value for label, value in timestamps.items() if value < project.audit_scope_start or value > project.audit_scope_end}
                if outside:
                    evidence = (
                        f"审计期间：{project.audit_scope_start.isoformat()} 至 {project.audit_scope_end.isoformat()}；"
                        + "；".join(f"{label}：{value.isoformat()}" for label, value in timestamps.items())
                        + "。文件系统时间仅为辅助线索，需结合文件内容、签署日期及取得记录判断。"
                    )
                    add_finding(
                        db,
                        run,
                        "ATT-004",
                        "medium",
                        att.index_no or att.title,
                        "资料文件创建/修改时间存在审计期间外日期，需核实资料形成及取得时点",
                        evidence,
                    )
                    count += 1
