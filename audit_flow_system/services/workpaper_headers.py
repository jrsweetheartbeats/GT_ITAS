...
HEADER_FIELDS: dict[str, list[str]] = {
    "project_name": ["项目名称"],
    "project_code": ["项目编号", "项目编码"],
    "oa_project_no": ["OA项目编号", "OA项目号"],
    "ims_project_no": ["IMS项目编号", "IMS项目号"],
    "audit_year": ["审计年度"],
...
LABEL_PATTERN = re.compile(
    r"(项目名称|项目编号|项目编码|OA\s*项目(?:编号|号)|IMS\s*项目(?:编号|号)|审计年度|被\s*审计\s*单位|客户名称|编制日期|编制人|复核日期|复核人|截止日|审计期间|索引号|页\s*次)(?:\s*[（(][^）)]*[）)])?(?:\s*[：:]\s*|\s+|$)"
)
LABEL_ONLY_PATTERN = re.compile(
    r"^\s*(项目名称|项目编号|项目编码|OA\s*项目(?:编号|号)|IMS\s*项目(?:编号|号)|审计年度|被\s*审计\s*单位|客户名称|编制日期|编制人|复核日期|复核人|截止日|审计期间|索引号|页\s*次)(?:\s*[（(][^）)]*[）)])?\s*[：:]?\s*$"
)
...
def project_date_text(value: Any) -> str:
    if not value:
        return ""
...
def project_date_chinese(value: Any) -> str:
    text = project_date_text(value)
...
def target_header_value(project: Project, workpaper: Workpaper, field: str, label: str, current_value: Any) -> str:
    ...
    if field == "project_name":
        return clean_cell_text(project.name)
    if field == "project_code":
        return clean_cell_text(project.code)
    if field == "oa_project_no":
        return clean_cell_text(project.oa_project_no)
    if field == "ims_project_no":
        return clean_cell_text(project.ims_project_no)
    if field == "audit_year":
        return clean_cell_text(project.audit_year)
    ...
    if field == "audit_period":
        ...
        if "审计期间" in label_text and project.audit_scope_start and project.audit_scope_end:
            return f"{project_date_text(project.audit_scope_start)}至{project_date_text(project.audit_scope_end)}"
        if project.audit_scope_end:
            return project_date_text(project.audit_scope_end)
...
def replace_project_placeholders(text: str, project: Project, workpaper: Workpaper) -> str:
    value = str(text or "")
    entity = clean_cell_text(project.entity_name)
    period_end = project_date_chinese(project.audit_scope_end)
    period_start = project_date_chinese(project.audit_scope_start)
    period = "至".join(part for part in [period_start, period_end] if part)
    replacements = {
        "【插入客户全称】": entity,
        "【插入客户简称】": entity,
        "[插入客户全称]": entity,
    ...
    if period_end:
        value = re.sub(r"[【\[]20(?:1|2)X年XX月XX日\s*[】\]]", period_end, value)
        value = value.replace("202X年12月31日", period_end).replace("201X年12月31日", period_end)
    if workpaper.code == "B60-2-3" and value.startswith("IT审计计划备忘录_"):
        return f"IT审计计划备忘录_{project.name}_{period or period_end}"
    return value


def _replace_docx_paragraph_text(paragraph: Any, value: str) -> None:
    if paragraph.runs:
...
def apply_docx_project_placeholders(path: Path, project: Project, workpaper: Workpaper) -> int:
    """Fill project-specific narrative placeholders that are not standard header labels."""
    doc = Document(path)
    changed = 0
    ...
    for paragraph in paragraphs:
        ...
        updated = replace_project_placeholders(current, project, workpaper)
        if updated == current:
            continue
        _replace_docx_paragraph_text(paragraph, updated)
        changed += 1
    if changed:
        doc.save(path)
    return changed
