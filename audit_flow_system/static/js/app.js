...
import { bindWorkpapers, loadWorkpaperTree, renderWorkpaperTemplateTree, renderWorkpapers } from './modules/workpapers.js?v=20260816b';
...
function primaryForSection(section) {
  if (['projectWorkspace', 'projects', 'clients', 'scopeCenter', 'pbcCenter', 'workpaperExecution', 'attachments', 'autoCheck', 'reviewCenter', 'findingKanban', 'workpapers', 'ruleVisualization', 'ruleInspection', 'reviews'].includes(section)) return 'projectWorkspace';
  if (['qualityDashboard', 'issueDashboard', 'projectOverview', 'quality', 'resourcePlan'].includes(section)) return 'qualityDashboard';
...
function bindGlobalCompatibility() {
  ...
  window.refreshLearning = () => loadLearning({weekId: state.selectedLearningWeekId});
}
