...
export function renderProjects() {
  fillSelect($('activeProject'), projectPickerRows(), p => projectSearchLabel(p), false);
  renderProjectPickerOptions();
  fillSelect($('priorProjectSelect'), state.projects, p => `${p.name}${p.audit_year ? ' / ' + p.audit_year : ''}`);
  fillSelect($('projectLeaderSelect'), state.users, u => u.display_name);
  fillSelect($('projectManagerSelect'), state.users, u => u.display_name);
  fillSelect($('qualityReviewerSelect'), state.users, u => u.display_name);
  ...
  const selected = state.projects.find(p => p.id === pid);
  const current = selected && (isActiveProjectStatus(selected.status) || Number(state.selectedProjectId || 0) === Number(selected.id))
    ? selected
    : activeProjectRows()[0] || state.projects[0];
  if (current) syncProjectPicker(current.id);
  if (!state.selectedProjectId && current) state.selectedProjectId = current.id;
