...
export function renderLearningDetail() {
  ...
  const verifyExecution = isSql && (question.validationRules?.verify_execution === true || question.validationRules?.exact_result === true);
  ...
  detail.innerHTML = `
    ...
    <div class="leetcode-question-layout" data-question-card="${question.id}">
      <section class="leetcode-problem-panel">
        ...
        ${verifyExecution ? '<div class="practice-notice success"><strong>自动判题</strong>：正式提交时会执行只读查询；字段名、字段顺序、结果行数、行顺序和每个字段值必须与后台标准答案完全一致。</div>' : ''}
