...
async function loadFindings(runId) {
  try {
    ...
    $('findingRows').innerHTML = rows.map(f => `
      <tr><td>${esc(f.rule_code)}</td><td>${tag(f.severity, statusClass(f.severity))}</td><td>${esc(f.target)}</td><td>${esc(f.issue)}</td><td>${esc(f.evidence)}</td></tr>
    `).join('') || '<tr><td colspan="5" class="empty">无复核问题</td></tr>';
  } catch (err) {
...
export async function runReview(useExternal) {
  const pid = activeProjectId();
  if (!pid) return setStatus('请先选择项目');
  setStatus('复核中');
  const run = await request('/api/review-runs', {
    method: 'POST',
    body: JSON.stringify({project_id: pid, use_external_rules: useExternal})
  });
