...
def list_clients(db: Session = Depends(get_db), user: User = Depends(current_user)) -> list[dict[str, Any]]:
    ...
    if not is_admin(user) and not (user.role and user.role.can_review):
        project_ids = visible_project_ids(db, user)
        client_ids = set(
            db.execute(
                select(Project.client_id).where(Project.id.in_(project_ids), Project.client_id.is_not(None))
            ).scalars().all()
