...
from ..core.security import can_view_project, current_user, ensure_project_viewer
...
OPEN_FINDING_STATUSES = {"open", "assigned", "retained", "revised", "待处理", "已分派", "保留", "已修订", "待复核"}
...
def workflow_dashboard(
    ...
    if activeOnly:
        projects = [row for row in projects if _is_active_project(row.status)]
    project_ids = {row.id for row in projects}
    workpapers = _rows_by_project(
        db.execute(select(Workpaper).where(Workpaper.project_id.in_(project_ids))).scalars().all() if project_ids else []
    )
    ...
    tasks = _rows_by_project(
        db.execute(select(Task).where(Task.project_id.in_(project_ids))).scalars().all() if project_ids else []
    )
    ...
    if project_ids:
        step_rows = db.execute(
            ...
            .where(Workpaper.project_id.in_(project_ids))
        ).all()
    ...
    alerts.sort(key=lambda item: (0 if item["level"] == "high" else 1, item["project_id"]))
...
def project_workflow_summary(
    ...
    workpapers = db.execute(select(Workpaper).where(Workpaper.project_id == project_id)).scalars().all()
    ...
    members = db.execute(select(ProjectMember).where(ProjectMember.project_id == project_id)).scalars().all()
...
def project_workpaper_execution_summary(
    ...
    if project is None:
        return {"items": []}
    ensure_project_viewer(db, project, user)
    workpapers = db.execute(select(Workpaper).where(Workpaper.project_id == project_id).order_by(Workpaper.stage, Workpaper.code)).scalars().all()
    attachment_counts = Counter(
        dict(
            db.execute(
                select(Attachment.workpaper_id, func.count(Attachment.id))
                .where(Attachment.project_id == project_id, Attachment.workpaper_id.is_not(None))
    ...
    for row in workpapers:
        ...
        data["is_done"] = _is_done(row.status)
