...
系统只支持 MariaDB。启动时会连接 MariaDB，并自动执行 `CREATE DATABASE IF NOT EXISTS ITA`；表结构必须通过 Alembic migration 建立和升级，应用启动和导入脚本不会自动建表。

连接 URL 由下列环境变量生成，仓库不提供用户名或密码默认值。
...
```bash
export AUDIT_FLOW_DB_URL='在部署环境中填写完整的mariadb+pymysql连接URL'
```
